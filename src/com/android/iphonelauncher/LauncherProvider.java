...
import com.android.iphonelauncher.LauncherSettings.Favorites;

public class LauncherProvider extends ContentProvider {
    private static final String TAG = "Launcher2.LauncherProvider";

    private static final boolean LOGD = false;

    private static final String DATABASE_NAME = "iphonelauncher.db";

    private static final int DATABASE_VERSION = 9;//8;

    static final String AUTHORITY = "com.android.iphonelauncher.settings";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private void convertWidgets(SQLiteDatabase db) {
            ...
            // buildOrWhereString(Favorites.ITEM_TYPE, bindSources);
            //
            // Cursor c = null;
            //
            // db.beginTransaction();
            ...
            // selectWhere, null, null, null, null);
            //
            // if (LOGD) Log.d(TAG, "found upgrade cursor count=" +
            // c.getCount());
            //
            // final ContentValues values = new ContentValues();
            // while (c != null && c.moveToNext()) {
         ...
         */
        private int loadFavorites(SQLiteDatabase db, boolean isDesktop) {
            ...
            try {
                XmlResourceParser parser = mContext.getResources().getXml(
                        isDesktop ? R.xml.default_workspace : R.xml.default_navigatebar);
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth)
                    ...
                    values.clear();
                    if(true){
                    	int container = a.getInt(R.styleable.Favorite_container, 
            						LauncherSettings.Favorites.CONTAINER_DESKTOP);
                    	if(container > 0){
                    		// folder
                    		Long id = mQsFolderInfo.get(container);
                    		if(id != null){
                    			container = id.intValue();
                    		} else {
                    			a.recycle();
                    			continue;
                    		}
	                    ...
	                    values.put(LauncherSettings.Favorites.CONTAINER,
	                            isDesktop ? LauncherSettings.Favorites.CONTAINER_DESKTOP
	                                    : LauncherSettings.Favorites.CONTAINER_NAVIGATEBAR);
                    }
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                    	...
                    	added = addFolderShortcut(db, values, a);
                    }
        ...
        private HashMap<Integer, Long> mQsFolderInfo = new HashMap<Integer, Long>();
        private boolean addFolderShortcut(SQLiteDatabase db, ContentValues values, TypedArray a) {
        	
        	//ActivityInfo info;
            //String packageName = a.getString(R.styleable.Favorite_packageName);
            String title = a.getString(R.styleable.Favorite_title);
            int folderId = a.getInteger(R.styleable.Favorite_defaultId, 1);
            //try {
            	
            	if(mQsFolderInfo.get(folderId) != null)
            		return false;
                ...
                mQsFolderInfo.put(folderId, db.insert(TABLE_FAVORITES, null, values));
                
            //} catch (PackageManager.NameNotFoundException e) {
            //    Log.w(TAG, "Unable to add folder: " + title, e);
             //   return false;
            //}
            return true;
        }
