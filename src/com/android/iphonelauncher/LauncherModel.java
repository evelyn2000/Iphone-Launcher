...
public class LauncherModel extends BroadcastReceiver {
	...
	static final String TAG = "Launcher2.Model";

	private static final int ITEMS_CHUNK = 16; // batch size for the workspace icons
	private int mBatchSize; // 0 is all apps at once
	private int mAllAppsLoadDelay; // milliseconds between batches
	private final boolean mSupportCustomOrder = false; //FeatureOption.MTK_YMCAPROP_SUPPORT
    ...
    private static final String STK_SELECTION = "com.mediatek.StkSelection";
    ...
    public interface Callbacks {
        ...
        //public void bindNavigateBarItems(ArrayList<ItemInfo> shortcuts);
    }

    LauncherModel(LauncherApplication app, IconCache iconCache) {
        mApp = app;
        SharedPreferences sharePref =  app.getSharedPreferences("firstflag", Context.MODE_PRIVATE);
        mIsFirstRunning = sharePref.getBoolean("isFirstRunFlag", true);
        if(mIsFirstRunning){
        	sharePref.edit().putBoolean("isFirstRunFlag", false).commit();
        }
        //android.util.Log.w("QsLog", "===mIsFirstRunning:"+mIsFirstRunning+"=====");
        mAllAppsList = new AllAppsList(iconCache);
        mIconCache = iconCache;

        mDefaultIcon = Utilities.createIconBitmap(app.getPackageManager().getDefaultActivityIcon(),
                app);
     ...
     */
    static void addOrMoveItemInDatabase(Context context, ItemInfo item, long container, int screen,
            int cellX, int cellY) {
    	addOrMoveItemInDatabase(context, item, container, screen, cellX, cellY, false);
    }
    ...
    static void addOrMoveItemInDatabase(Context context, ItemInfo item, long container, int screen,
        ...
        if (item.container == ItemInfo.NO_ID) {
            // From all apps
            addItemToDatabase(context, item, container, screen, cellX, cellY, notify);
        } else {
            // From somewhere else
            moveItemInDatabase(context, item, container, screen, cellX, cellY, notify);
        }
    }
    ...
    static void moveItemInDatabase(Context context, ItemInfo item, long container, int screen,
        ...
        cr.update(LauncherSettings.Favorites.getContentUri(item.id, notify), values, null, null);
    }
     ...
     * item.
     */
    static boolean addItemToDatabase(Context context, ItemInfo item, long container, int screen,
            int cellX, int cellY, boolean notify) {
        item.container = container;
        item.screen = screen;
        item.cellX = cellX;
...
//        		notify = true;
//        	//mFolders
//        	//LauncherApplication app = ((LauncherApplication) context.getApplication());
//        	//context.getp
//        }
    ...
    static void deleteItemFromDatabase(Context context, ItemInfo item) {
        ...
        cr.delete(LauncherSettings.Favorites.getContentUri(item.id, true), null, null);
    }
    ...
    static void deleteUserFolderContentsFromDatabase(Context context, UserFolderInfo info) {
        ...
        cr.delete(LauncherSettings.Favorites.CONTENT_URI, LauncherSettings.Favorites.CONTAINER
                + "=" + info.id, null);
    }
...
//    }
    ...
    public void startLoader(Context context, boolean isLaunching) {
        synchronized (mLock) {
            ...
            if (mCallbacks != null && mCallbacks.get() != null) {
                ...
                AllAppsList.loadIngorePackage(context);
    ...
    private class LoaderTask implements Runnable {
        ...
        private boolean isShortCutExisting(ItemInfo info){
        	for(ItemInfo item : mItems){

				if (info.container < 0 && info.cellX == item.cellX && item.cellY == info.cellY
						&& item.screen == info.screen
						&& item.itemType == info.itemType
						&& item.container == info.container) {
//					if(DEBUG_LOADERS){
//		            	Log.i(TAG, "isShortCutExisting(0)==="+info.toString());
//		            	Log.i(TAG, "isShortCutExisting(1)==="+item.toString());
//					}
					return true;
				}
				...
				if (item.equals(info)){
//					if(DEBUG_LOADERS){
//						Log.i(TAG, "isShortCutExisting(2)==="+info.toString());
//		            	Log.i(TAG, "isShortCutExisting(3)==="+item.toString());
//					}
					return true;
				}
        	}
        ...
        private void loadWorkspace() {
			synchronized (LauncherModel.mObject) {
	            ...
	            mItems.clear();
	            //mQsNavigateBarItems.clear();
	            mFolders.clear();
	            //mAllAppsList.clear();
	            if(DEBUG_LOADERS)
	            	Log.w(TAG, "loadWorkspace(0)=====");
	            final ArrayList<Long> itemsToRemove = new ArrayList<Long>();
	
//	            final Cursor c = contentResolver.query(
//	                    LauncherSettings.Favorites.CONTENT_URI, null, null, null, 
//	                    LauncherSettings.Favorites.CONTAINER+" ASC," 
//	                    + LauncherSettings.Favorites.SCREEN+" ASC," 
//	                    + LauncherSettings.Favorites._ID + " ASC");
	            
	            final Cursor c = contentResolver.query(
	                    LauncherSettings.Favorites.CONTENT_URI, null, null, null, 
	                    LauncherSettings.Favorites.SCREEN+" ASC, " 
	                    + LauncherSettings.Favorites.CELLY + " ASC, "
	                    + LauncherSettings.Favorites.CELLX + " ASC");
...
//	                final int appOrderIdIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.APPORDER_ID);
//	                
//	                final int appFlagsIndex = c.getColumnIndexOrThrow(
//	                        LauncherSettings.Favorites.APP_FLAGS);
	                    ...
	                    try {
	                        ...
	                        case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
	                            ...
	                            if (info != null) {
	                                ...
	                                //info.appOrderId = c.getInt(appOrderIdIndex);
...
//	                                }
	                                ...
	                                if(isShortCutExisting(info)){
                                		Log.e(TAG, "Error loading application existing " + info.toString() + ", removing it");
...
//	                                            id, false), null, null);
	                        ...
	                        case LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER:
	                            ...
	                            //folderInfo.appOrderId = c.getInt(appOrderIdIndex);
...
//	                            }
	                            ...
	                            if(isShortCutExisting(folderInfo))
	                            	break;
	
	                            switch (container) {
	                                case LauncherSettings.Favorites.CONTAINER_DESKTOP:
	                                case LauncherSettings.Favorites.CONTAINER_NAVIGATEBAR:
	                                	//mAllAppsList.addItem(folderInfo);
	                                	mItems.add(folderInfo);
	                                    break;
	                            }
	
	                            //mAllAppsList.addFolder(folderInfo.id, folderInfo);
	            ...
	            //Log.w(TAG, "loadWorkspace(2)=====mItems:"+mItems.size()+"==mFolders:"+mFolders.size()
	           // 		+"==mIsFirstRunning:"+mIsFirstRunning);
	            // remove empty item
	            if(mIsFirstRunning){
		            int nPrevScreen = 0;
		            int nStartCellInfo = 0;
		            for(ItemInfo item : mItems){
	
						if (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
							if(nPrevScreen != item.screen){
								nPrevScreen = item.screen;
								nStartCellInfo = 0;
							}
							final int curCell = item.cellY * 4 + item.cellX;
							//final int space = curCell - nStartCellInfo;
							
							if(curCell != nStartCellInfo){
								//nFirstEmptyCellInfo = nPrevCellInfo + 1;
								item.cellX = nStartCellInfo%4;
								item.cellY = nStartCellInfo/4;
								...
								//moveItemInDatabase(context, item, item.container, item.screen, item.cellX, item.cellY);
							}
							nStartCellInfo++; 
						}
		        	}
	          ...
	          //mFolders
        	    for(Map.Entry<Long, FolderInfo> entry : mFolders.entrySet()){   
        	         
        	    	UserFolderInfo folder = (UserFolderInfo)(entry.getValue());
        	    	if(folder.size() <= 0)
        	    		itemsToRemove.add(folder.id);
        	    }
...
//	            }
        ...
        private void bindWorkspace() {
            ...
            if(true){
	            final int currentScreen = 1;//oldCallbacks.getCurrentWorkspaceScreen();
	            final ArrayList<ItemInfo> currentPageArray = new ArrayList<ItemInfo>();
	            final ArrayList<ItemInfo> otherPageArray = new ArrayList<ItemInfo>();
	            
	            // once for the current screen
	            //for (int i=0; i<N; i++) {
	            for(ItemInfo item : mAllAppsList.data){
	                //final ItemInfo item = mAllAppsList.get(i);
	            	if(DEBUG_LOADERS)
	            		Log.w(TAG, "bindWorkspace(0)==="+item.toString());
	                if (item.screen == currentScreen) {
	                	currentPageArray.add(item);
	                } else {
	                	otherPageArray.add(item);
	                }
	            }
	            if(DEBUG_LOADERS)
	            	Log.e(TAG, "bindWorkspace(0)===currentScreen:"+currentScreen+"=currentPageArray:"+currentPageArray.size()+"==otherPageArray:"+otherPageArray.size());
	            mHandler.post(new Runnable() {
	                public void run() {
	                    Callbacks callbacks = tryGetCallbacks(oldCallbacks);
	                    if (callbacks != null) {
	                        callbacks.bindItems(currentPageArray, 0, currentPageArray.size());
	                    }
	                }
	            });
	            
	            if(DEBUG_LOADERS)
	            	Log.e(TAG, "bindWorkspace(1)====currentPageArray:"+currentPageArray.size()+"==otherPageArray:"+otherPageArray.size());
	            // Add the items to the workspace.
	            N = otherPageArray.size();
	            for (int i=0; i<N; i+=ITEMS_CHUNK) {
	                ...
	                mHandler.post(new Runnable() {
	                    public void run() {
	                        Callbacks callbacks = tryGetCallbacks(oldCallbacks);
	                        if (callbacks != null) {
	                            callbacks.bindItems(otherPageArray, start, start+chunkSize);
	                        }
	                    }
	                });
	            }
	            ...
	            for (int i=0; i<N; i+=ITEMS_CHUNK) {
	                ...
	                mHandler.post(new Runnable() {
	                    public void run() {
	                        Callbacks callbacks = tryGetCallbacks(oldCallbacks);
	                        if (callbacks != null) {
	                            callbacks.bindItems(mAllAppsList.data, start, start+chunkSize);
	                        }
	                    }
	                });
	            }
...
//            
//            // shallow copy
//            final ArrayList<ItemInfo> list
//                    = (ArrayList<ItemInfo>)mAllAppsList.data.clone();
//            mHandler.post(new Runnable() {
//                public void run() {
//                    final long t = SystemClock.uptimeMillis();
...
//            });
        ...
        private void loadAllAppsByBatch() {
            ...
            if(true){
            	...
            	ArrayList<ItemInfo> itemsToRemove = (ArrayList<ItemInfo>)mItems.clone();
            	// remove all folder info
            	for(int i=itemsToRemove.size()-1; i>=0; i--){
        			ItemInfo item = itemsToRemove.get(i);
        			//Log.w(TAG, "loadAllAppsByBatch(0)==index:" + i +"==type:"+item.itemType);
            	...
            	if(DEBUG_LOADERS)
            		Log.w(TAG, "loadAllAppsByBatch(1)==itemsToRemove size:" + itemsToRemove.size()+"==mItems size:"+mItems.size()+"==apps size:"+apps.size());
            	
            	for(ResolveInfo app : apps){
            		
            		final ComponentName componentName = new ComponentName(app.activityInfo.applicationInfo.packageName, 
            				app.activityInfo.name);
            		
            		AppShortcutInfo savedItemInfo = null;
            		
            		for(int i=itemsToRemove.size()-1; i>=0 && !mStopped; i--){
            			AppShortcutInfo item = (AppShortcutInfo)itemsToRemove.get(i);
            		...
            		if(STK_SELECTION.equals(app.activityInfo.applicationInfo.packageName)
            				&& !hasSimCard()){
            			continue;
            		}
            		...
            		if(savedItemInfo != null){
            			savedItemInfo = new ShortcutInfo((ShortcutInfo)savedItemInfo);
            			mIconCache.getTitleAndIcon(savedItemInfo, app);
            		} else {
            			savedItemInfo = new ShortcutInfo(packageManager, app, componentName, mIconCache);
            			//mAllAppsList.add(new ApplicationInfo(packageManager, componentName, app, mIconCache, savedItemInfo));
            		}
            		
            		mAllAppsList.addItem(savedItemInfo);
            	}
            	if(DEBUG_LOADERS)
            		Log.w(TAG, "loadAllAppsByBatch(2)==itemsToRemove size:" + itemsToRemove.size()+"==apps size:"+mItems.size()+"==mAllAppsList size:"+mAllAppsList.size());
            	if(!mStopped){
	            	...
	            	if(itemsToRemove.size() > 0){
	            		...
	            		ContentProviderClient client = mContext.getContentResolver().acquireContentProviderClient(
                                LauncherSettings.Favorites.CONTENT_URI);
		                // Remove dead items
		                for (ItemInfo item : itemsToRemove) {
		                	if(DEBUG_LOADERS)
		                	//	Log.w(TAG, "loadAllAppsByBatch(4)==remove item id:" + item.id+"==container:"
			                //	+item.container
			                //	+"=="+item.toString());
		                	
		                	if(item.container > 0){
		                		UserFolderInfo folder = (UserFolderInfo)mFolders.get(item.container);
		                		if(folder != null){
		                			folder.remove((AppShortcutInfo)item);
		                			if(DEBUG_LOADERS)
		                				Log.e(TAG, "loadAllAppsByBatch(5)==remove from folder:" + item.toString()+"==folder size:"+folder.size());
		                			if(folder.size() <= 0){
		                				
		                				try {
					                        client.delete(LauncherSettings.Favorites.getContentUri(folder.id, false),
					                                null, null);
					                    } catch (RemoteException e) {
					                        //Log.w(TAG, "Could not remove id = " + id);
					                    }
		                	...
		                	if(item.id != ItemInfo.NO_ID){
			                    // Don't notify content observers
		                		if(DEBUG_LOADERS)
		                    		Log.w(TAG, "loadAllAppsByBatch(6)===delete item===="+item.toString());
		                		
			                    try {
			                        client.delete(LauncherSettings.Favorites.getContentUri(item.id, false),
			                                null, null);
			                    } catch (RemoteException e) {
			                        //Log.w(TAG, "Could not remove id = " + id);
			                    }
	            	...
	            	if(DEBUG_LOADERS)
	            		Log.w(TAG, "loadAllAppsByBatch(3)====apps size:"+mItems.size()+"==mAllAppsList size:"+mAllAppsList.size());
	            	//mAllAppsList.reorderApplist();
	            	
	            	mAllAppsList.removeIngoreApplist();
            	}
...
//	            int N = 0;
//	            ArrayList<ComponentName> matchedApps = new ArrayList<ComponentName>();
//	
//	            if(!mStopped && apps != null){
//	            	N = apps.size();
//	            }
//	            
...
//	//            	if(mAllAppsList.updateApplication(packageManager, componentName, info)){
//	//            		mAllAppsList.added.add(new ApplicationInfo(packageManager, componentName, info, mIconCache));
//	//            	}
//	            	mAllAppsList.updateApplication(packageManager, componentName, info);
//	            	matchedApps.add(componentName);
//	            }
//	            
//	            if(!mStopped){
//	            	mAllAppsList.mergeAllPackagesToDataArray(matchedApps);
//	            }
            }
        }
    ...
    private class PackageUpdatedTask implements Runnable {
        ...
        public void run() {
            ...
            switch (mOp) {
                case OP_ADD:
                    for (int i=0; i<N; i++) {
                        ...
                        mAllAppsList.addPackage(packageManager, context, packages[i]);
                    }
                    break;
                case OP_UPDATE:
                    for (int i=0; i<N; i++) {
                        if (DEBUG_LOADERS) Log.d(TAG, "mAllAppsList.updatePackage " + packages[i]);
                        mAllAppsList.updatePackage(packageManager, context, packages[i]);
                    }
                    break;
                case OP_REMOVE:
                case OP_UNAVAILABLE:
            ...
            ArrayList<ItemInfo> removed = null;
            ...
            if (mAllAppsList.removed.size() > 0) {
                ...
                for (ItemInfo info: removed) {
                	if(info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION)
                		mIconCache.remove(((AppShortcutInfo)info).getComponent());
                }
            }
            ...
            if (added != null) {
                final ArrayList<ItemInfo> addedFinal = added;
                mHandler.post(new Runnable() {
                    public void run() {
            ...
            if (removed != null) {
                ...
                final ArrayList<ItemInfo> removedFinal = removed;
                mHandler.post(new Runnable() {
                    public void run() {
                    	if (callbacks == mCallbacks.get()) {
    ...
    public void changeSmsShortcutIcon(ComponentName component, Bitmap icon) {
        if (isDesktopLoaded()) {
            int ncount = mAllAppsList.data.size();
            // Log.d("QiShang", "changeSmsShortcutIcon()==ncount:"+ncount+"==");
            for (int i = 0; i < ncount; i++) {
                final ItemInfo info = mAllAppsList.data.get(i);
                if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                    // final ResolveInfo resolveInfo =
                    // manager.resolveActivity(((ApplicationInfo)info).intent,
                    // 0);
                    // final ActivityInfo activityInfo =
                    // resolveInfo.activityInfo; ShortcutInfo
                    if (((AppShortcutInfo) info).equals(component)) {
                        ((AppShortcutInfo) info).setIcon(icon);
                        // Log.d("QiShang",
                        // "changeSmsShortcutIcon()==change shortcut success==");
                    }
                }
            }
        }
        /*
         * else { Log.d("QiShang",
         * "changeSmsShortcutIcon()==isDesktopLoaded is false=="); }
         */
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Context context,
        ...
        if (resolveInfo != null) {
            // final Callbacks cbk = mCallbacks.get();
            // if (cbk != null)
            // icon = cbk.getUnReadSmsCountIcon(componentName);

            if (icon == null)
                icon = mIconCache.getIcon(componentName, resolveInfo);
        } else {
            // if have not match activity return null
            return null;
        }
        // the db
        if (icon == null) {
...
//     */
//    public ApplicationInfo getApplicationInfo(PackageManager manager, Intent intent, Context context,
//            Cursor c, int iconIndex, int titleIndex/*, int appFlagsIndex*/) {
//        Bitmap icon = null;
...
//            icon = getIconFromCursor(c, iconIndex);
//            info.title = c.getString(titleIndex);
//            //info.flags = c.getInt(appFlagsIndex);
//        }
//        
//        // from the db
//        if (TextUtils.isEmpty(info.title) || icon == null) {
//        	final ResolveInfo resolveInfo = manager.resolveActivity(intent, 0);
//        	
//        	if (resolveInfo != null) {
//                info.title = resolveInfo.activityInfo.loadLabel(manager);
...
//    }
    ...
    ShortcutInfo addShortcut(Context context, Intent data, CellLayout.CellInfo cellInfo,
            boolean notify) {
...
//                cellInfo.screen, cellInfo.cellX, cellInfo.cellY, notify);
    ...
    // private static void loadLiveFolderIcon(Context context, Cursor c, int
    // iconTypeIndex,
    // int iconPackageIndex, int iconResourceIndex, LiveFolderInfo
    // liveFolderInfo) {
    //
    // int iconType = c.getInt(iconTypeIndex);
    // switch (iconType) {
    ...
    // final int id = resources.getIdentifier(resourceName, null, null);
    // liveFolderInfo.icon =
    // Utilities.createIconBitmap(resources.getDrawable(id),
    // context);
    // } catch (Exception e) {
    // liveFolderInfo.icon = Utilities.createIconBitmap(context.getResources()
    // .getDrawable(R.drawable.ic_launcher_folder), context);
    // }
     ...
     * previously, or make a new one.
     */
    // private static LiveFolderInfo findOrMakeLiveFolder(HashMap<Long,
    // FolderInfo> folders, long id) {
    // // See if a placeholder was created for us already
    // FolderInfo folderInfo = folders.get(id);
    ...
    public static final Comparator<AppShortcutInfo> APP_NAME_COMPARATOR = new Comparator<AppShortcutInfo>() {
        public final int compare(AppShortcutInfo a, AppShortcutInfo b) {
...
//        ApplicationInfo
//                .dumpApplicationInfoList(TAG, "mAllAppsList.modified", mAllAppsList.modified);
//        mLoader.dumpState();
    }


    public ShortcutInfo getShortcutInfoFromTaskInfo(PackageManager manager,
            RunningTaskInfo taskInfo, Context context) {
        Bitmap icon = null;
        final ShortcutInfo info = new ShortcutInfo();
        ...
        info.customIcon = true;
        info.setActivity(componentName, Intent.FLAG_ACTIVITY_LAUNCHED_FROM_HISTORY
                | Intent.FLAG_ACTIVITY_NEW_TASK);
        return info;
    }

    public void setLocaleChange(boolean change) {
        mLocaleChange = change;
    }

    private boolean hasSimCard() {
        return SIMInfo.getInsertedSIMCount(mApp) > 0;
    }
}
